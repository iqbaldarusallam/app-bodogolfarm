...
export async function getByLivestock(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    const logs = await feedingService.getByLivestock(req.params.livestockId);
...
export async function getById(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    const log = await feedingService.getById(req.params.id);
...
export async function update(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    const log = await feedingService.update(req.params.id, req.body);
    res.status(200).json({ success: true, data: log });
  } catch (error) {
    next(error);
  }
...
export async function remove(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    const result = await feedingService.remove(req.params.id);
